...
class Payment(models.Model):
    ...
    STATUS_TRANSLATIONS = {
        ...
        STATUS_ERROR : _('Payment process has encounterd an error'),
        STATUS_CANCELED : _('Payment has been canceled'),
    ...
    status = models.PositiveSmallIntegerField(_('status'), choices=STATUS_CHOISES, default=STATUS_VOID, db_index=True)
    device_uuid = models.CharField(_('device uuid'), max_length=40, blank=True)
    phone_number = models.BigIntegerField(_('phone number'), validators=[validate_phone_number], blank=True, db_index=True)
